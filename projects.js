...
function showProjectModal(projectId) {
    ...
    if (project.whatItIs) {
        content += `<h3>What it is</h3><p>${project.whatItIs}</p>`;
    }
    
    if (project.myContributions) {
        content += `<h3>My contributions</h3>${generateList(project.myContributions)}`;
    }

    if (project.howItWorks) {
        content += `<h3>How it works</h3>${generateList(project.howItWorks)}`;
    }

    if (project.pipelineWorkflow) {
        content += `<h3>Pipeline Workflow</h3>${generateList(project.pipelineWorkflow)}`;
    }

    if (project.useCaseExample) {
        content += `<h3>${project.useCaseExample.title || 'Use Case Example'}</h3>`;
        if (project.useCaseExample.description) {
            content += `<p>${project.useCaseExample.description}</p>`;
        }
        content += `${generateList(project.useCaseExample.steps)}`;
    ...
    if (project.keyFeatures) {
        content += `<h3>Key Features</h3>${generateList(project.keyFeatures)}`;
    }

    if (project.impact) {
        content += `<h3>${project.impact.title || 'Impact'}</h3>${generateTable(project.impact)}`;
    }
    
    if (project.productionEfficiency) {
        content += `<h3>${project.productionEfficiency.title || 'Production Efficiency'}</h3>${generateTable(project.productionEfficiency)}`;
    }

    if (project.myRole) {
        content += `<h3>My Role</h3><p>${project.myRole}</p>`;
    }
    ...
    if (stack) {
        content += `<h3>${stackTitle}</h3><div class="tech-stack">${stack.map(tech => `<span>${tech}</span>`).join('')}</div>`;
    }
